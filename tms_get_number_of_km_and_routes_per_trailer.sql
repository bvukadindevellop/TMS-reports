-- Function: tms_get_number_of_km_and_routes_per_trailer(integer)

-- DROP FUNCTION tms_get_number_of_km_and_routes_per_trailer(integer);

CREATE OR REPLACE FUNCTION tms_get_number_of_km_and_routes_per_trailer(IN treil_id integer)
  RETURNS TABLE(mesec character varying, ukupno_km numeric, ukupno_km_pre_god_dana numeric, broj_tura integer, broj_tura_pre_god_dana integer) AS
$BODY$
begin
...
and t_working_order.trailer_id = treil_id
...
order by tmp1.id;
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION tms_get_number_of_km_and_routes_per_trailer(integer)
  OWNER TO postgres;
